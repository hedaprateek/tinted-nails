...
- **Four colour themes**, remembered per visitor in `localStorage`: Warm (default), Blush, Ember, Midnight. A palette sets neutrals and section washes only — never the accent, which always comes from the selected lacquer. Dark-mode visitors open on Ember, everyone else on Warm. Print overrides every token with `!important`, since the picker writes them inline on `:root`.
